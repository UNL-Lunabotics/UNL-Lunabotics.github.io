...
To do this, open your terminal and type in the following command: `ssh workstation@TAILSCALE_IP_HERE`, where `TAILSCALE_IP_HERE` is the IP Address that Tailscale copied to your clipboard in the [previous step]({% link docs/Technical/Setup Dev Tools/Remote/Tailscale-Setup.md %}#using-tailscale). Then type `INSERTUNL'SDEFAULTPASSWORDASKIFYOUDONTKNOWIT` for the password when it asks.
