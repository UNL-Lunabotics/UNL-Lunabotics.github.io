...
- **Boolean**: A simple data type with only two possible values: *true*, and *false*. These two values represent the two truth values of logic and Boolean Algebra (Source: [Wikipedia](https://en.wikipedia.org/wiki/Boolean_data_type)).
- **Branch** [[Control Flow](#c)]: In a computer program, a branch instruction causes a computer program to deviate from its default [control flow](#c) and begin executing a different set of instructions. One of the most common branch instructions in programming are conditionals such as `if` statements.
- **Branch** [[Version Control](#v)]: Branching is the act of duplicating an object under version control, usually a [repository](#r). The two identical objects can then be modified separately and in parallel so as to not conflict with one another. These two diverging objects are called branches.
...
- **Clone** [[Version Control](#v)]: A clone is a copy of a remote [repository](#r) you make on your local system through a process called [cloning](https://github.com/git-guides/git-clone).
...
- **Commit** [[Version Control](#v)]: A commit is effectively an operation that finalizes changes in a version control system. This makes the commit part of the [repository's](#r) permanent history (Source: [Wikipedia](https://en.wikipedia.org/wiki/Commit_(version_control))).
...
- **Fork** [[Version Control](#v)]: A fork is a duplicate of an existing [repository](#r) that has all of its contents and history stored completely separate from the original repository. It is very similar to a [branch](#b), with the main difference being that a branch is used for work within the same repository, whereas a fork creates an entirely new repository.
...
- **Instance**: An instance is one individual copy of something built from a blueprint. An example of this in computer science is an [object](#o) built based on specifications from its associated [class](#c).
...
- **JavaScript Object Notation (JSON)**: A data-serialization language similar to [XML](#x), but utilizing syntax similar to that used to construct objects in JavaScript. This makes the language both human-readable and very easy for computers to parse, even faster than XML.

### K

- **Kebab Case**: Also sometimes called dash case, this is a grammar convention used in many programming languages characterized by the use of lowercase letters and hyphen characters in place of whitespace for multi-word phrases. Often stylized as `kebab-case`.
...
- **Machine Code**: Machine code is the raw instructions fed to a computer's central processing unit (CPU) in order to execute tasks. It is generally not human readable. Different CPU architectures use different machine code.
- **Merge** [[Version Control](#v)]: In version control, merging (also called integration) is a fundamental operation that reconciles changes made to a version-controlled collection of files (Source: [Wikipedia](https://en.wikipedia.org/wiki/Merge_(version_control))). It effectively takes changes from two [branches](#b) and combines them into one branch, giving the user the opportunity to reconcile any conflicts between the two branches.
...
- **Multithreading**: Multithreading is a programming model that allows for multiple processes to be executed at once utilizing a multithreaded central processing unit (CPU). This is especially useful for complex computations that would take a very long time if each step had to be executed one after the other.

### N

- **Nav2**: Nav2 is a robotics navigation framework that provides various autonomous functions to a robot. It allows robots to navigate through complex environments and complete user-defined application tasks (Source: [Nav2 Docs](https://docs.nav2.org)).
...
- **Pull Request (PR)** [[Version Control](#v)]: The pull request is the primary way that contributions are made to [repositories](#r) utilizing a [version control](#v) system. When making a PR, the contributor requests that the project maintainer "pull" the changes. The maintainer has to [merge](#m) the pull request if the contribution should become part of the source codebase (Source: [Wikipedia](https://en.wikipedia.org/wiki/Distributed_version_control#Pull_requests)).
...
- **Repository (Repo)**: A software repository is a storage location for software packages, usually managed by a [version control](#v) system like Git.
...
- **Robot Visualization Tool (RViz)**: A highly configurable three-dimensional visualizer used to visualize robots, the environments they work in, and sensor data.
...
- **Service** [[ROS2](#r)]: One of the three forms of communication between [nodes](#n) in ROS2. Services follow a call-and-response model, where information is only provided to the caller (or [client-side](#c)) upon request from the [server-side](#s) (Source: [ROS2 Docs](https://docs.ros.org/en/jazzy/Tutorials/Beginner-CLI-Tools/Understanding-ROS2-Services/Understanding-ROS2-Services.html)). This distinguishes it from [topics](#t), wherein information is constantly being provided and received via publishers and subscribers.
...
- **Software Development Kit (SDK)**: A collection of software development tools bundled into one installable package. These tools are generally platform/operating system specific, as different platforms will have different tools for developing software.
...
- **Version Control**: The practice of controlling, organizing, and tracking different versions in the history of computer files (Source: [Wikipedia](https://en.wikipedia.org/wiki/Version_control)). This process is most often automated using version control systems like [Git](#g).

### W

- **Workspace** [[ROS2](#r)]: A workspace is a directory on your computer containing ROS2 packages (Source: [ROS2 Docs](https://docs.ros.org/en/jazzy/Tutorials/Beginner-Client-Libraries/Creating-A-Workspace/Creating-A-Workspace.html)).
