...
4. The use of 2.4G Zigbee/IEEE 802.15.4 technology is prohibited
5. The SSID name of the bands should be `Team_##` for the 2.4G bands and `Team_##_5G` for the 5G bands.
6. The SSID has to be broadcasting. Ensure any 'Hide SSID' options are UNCHECKED.
7. The WiFi bands have to be encrypted (means unspecified).
8. On the 2.4G band, the Channel Width has to be in the 20MHz range due to the no channel bonding rule.
9. The 2.4G band has to be on Channel 1 during competition runs and inspection.
10. The robot CAN NOT be on the 2.4G band in the RoboPit (unless special permission from the Pit Boss is gained. Just use the 5G band for testing).
11. Any wireless back channels are completely banned. No bridging connections from the robot to the router via secondary wireless devices.
